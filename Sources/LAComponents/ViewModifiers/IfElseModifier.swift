...
extension View {
  ...
  public func ifElseModifier<M1, M2>(for condition: Binding<Bool>, if trueModifier: M1, else falseModifier: M2) -> some View where M1: ViewModifier, M2: ViewModifier {
    Group {
  ...
  ///     - trueModifier: A *ViewModifier* struct apply to the **if case**.
  ///     - falseModifier: A *ViewModifier* struct apply to the **else case**.
  public func ifElseModifier<M1, M2>(for condition: Bool, if trueModifier: M1, else falseModifier: M2) -> some View where M1: ViewModifier, M2: ViewModifier {
    Group {
      if condition {
        self.modifier(trueModifier)
      } else {
